...
// creates initial node array of huffman_node* (huf_node)
...
// traversal to ASSIGN CODES TO LEAF nodes of HUFFMAN TREE
void huffman::traverse(huf_node node, string code)
{
...
// recreates HUFFMAN tree during decompresssion
inline void huffman::build_tree(string& path, char a_code)
{
	huf_node current = root;
...
// huffman class (header file) constructor overloading
...
// function to create the priority queue of {unique char,freq}
void huffman::make_pq()
{
...
// function to create the HUFFMAN TREE using the priority queue
// creates the huffman tree with highest frequecy character being leaf from bottom to top
void huffman::create_huffman_tree()
{
	priority_queue<huf_node, vector<huf_node>, compare> temp(pq);
	while (temp.size() > 1)
	{
...
// function to assign codes for each leaf in the huffman tree
void huffman::assign_huffman_codes()
{
	traverse(root, "");
}

// use the huffman tree, assigned codes to encode the file, essentially compressing it
void huffman::encode_file()
{
	...
	while (!temp.empty()) //get all characters and their huffman codes for output
	{
		...
		string tmpSubstr = s.substr(0,8);
		in += (char)binary_to_decimal(tmpSubstr);
		// in+=' | ';
		for (int i = 0; i < 15; i++)
...
// function to recreate the huffman tree from the compressed file
...
// function to decode the compressed file
void huffman::decode_file()
{
	in_file.open(in_file_name, ios::in | ios::binary);
